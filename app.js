...
async function refreshStats() {
  try {
    ...
    updateChart('chartEstados', leads.porEstado || [], 'Doughnut');
    updateChart('chartFuentes', leads.porFuente || [], 'Pie');
    
    // Campaign stats
    document.getElementById('cTotal').textContent = campaign.total || 0;
    document.getElementById('cEnviados').textContent = campaign.enviados || 0;
    document.getElementById('cPendientes').textContent = campaign.pendientes || 0;
    document.getElementById('cRebotados').textContent = campaign.rebotados || 0;
    document.getElementById('cBajas').textContent = campaign.bajas || 0;
...
function updateChart(id, data, type) {
  ...
  const colors = labels.map(l => id === 'chartEstados' ? getStatusColor(l) : getSourceColor(l));
